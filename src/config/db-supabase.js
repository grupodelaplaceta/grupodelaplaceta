...
function ejecutarMigracionesSqlite(database) {
  ...
  database.run(`CREATE TABLE IF NOT EXISTS consentimientos (id INTEGER PRIMARY KEY AUTOINCREMENT, usuario_id INTEGER NOT NULL, tipo TEXT, aceptado INTEGER DEFAULT 0, fecha_aceptacion TEXT, ip_aceptacion TEXT, version TEXT DEFAULT '1.0', documento TEXT, creado_en TEXT DEFAULT (datetime('now')))`);
  ...
  try { database.run(`ALTER TABLE consentimientos ADD COLUMN version TEXT DEFAULT '1.0'`); } catch(e) {}
  try { database.run(`ALTER TABLE consentimientos ADD COLUMN documento TEXT`); } catch(e) {}
